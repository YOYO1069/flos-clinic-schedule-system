...
export function AppointmentEditDialog({ 
  ...
  useEffect(() => {
    if (appointment) {
      console.log('📝 編輯預約:', appointment)
      
      setFormData({
        taiwan_date: appointment.taiwan_date || appointment.appointment_date || '',
        time_24h: appointment.time_24h || appointment.appointment_time || '',
        customer_name: appointment.customer_name || '',
        customer_birthday: appointment.customer_birthday || '',
        contact_phone: appointment.contact_phone || '',
        treatment_item: appointment.treatment_item || '',
        room_used: appointment.room_used || '',
        equipment_used: appointment.equipment_used || '',
        customer_source: appointment.customer_source || '',
        appointment_status: appointment.appointment_status || '尚未報到',
        data_source: appointment.data_source || '手動新增',
        consultant: appointment.consultant || '',
        assistant: appointment.assistant || '',
        attending_physician: appointment.attending_physician || '',
        duration_hours: appointment.treatment_duration_hours || appointment.duration_hours || 1.0,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error('更新預約失敗：' + error.message)
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error('刪除預約失敗：' + error.message)
    } finally {
      setDeleting(false)
    }
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-gray-900 border-gray-700">
        <DialogHeader>
          <DialogTitle className="text-white text-xl">編輯預約</DialogTitle>
